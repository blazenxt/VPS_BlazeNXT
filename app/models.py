...
class NotificationPreference(Base):
    ...
    id:Mapped[int]=mapped_column(primary_key=True); user_id:Mapped[int]=mapped_column(ForeignKey('users.id',ondelete='CASCADE'),unique=True,index=True); web_enabled:Mapped[bool]=mapped_column(Boolean,default=True); email_enabled:Mapped[bool]=mapped_column(Boolean,default=True); telegram_enabled:Mapped[bool]=mapped_column(Boolean,default=True); push_enabled:Mapped[bool]=mapped_column(Boolean,default=True); event_categories:Mapped[str]=mapped_column(Text,default='["deployment","security","billing","support","incident"]'); updated_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=now,onupdate=now)
class PushSubscription(Base):
    __tablename__='push_subscriptions'
    id:Mapped[int]=mapped_column(primary_key=True); user_id:Mapped[int]=mapped_column(ForeignKey('users.id',ondelete='CASCADE'),index=True); endpoint:Mapped[str]=mapped_column(Text,unique=True); p256dh:Mapped[str]=mapped_column(Text); auth:Mapped[str]=mapped_column(Text); user_agent:Mapped[str|None]=mapped_column(String(300)); enabled:Mapped[bool]=mapped_column(Boolean,default=True,index=True); created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=now); last_success_at:Mapped[datetime|None]=mapped_column(DateTime(timezone=True)); last_error:Mapped[str|None]=mapped_column(Text)
