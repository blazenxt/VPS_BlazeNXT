...
from app.models import ApiKey,AuthIdentity,AuthTokenUse,User,UserSecurity
...
def login_response(uid,db):
    security=db.scalar(select(UserSecurity).where(UserSecurity.user_id==uid,UserSecurity.enabled==True))
    if security:
        r=RedirectResponse('/auth/2fa',303);r.set_cookie('blaze_preauth',sign_preauth(uid),httponly=True,secure=s.production,samesite='lax',max_age=300);return r
...
async def google_callback(request:Request,code:str,state:str,db:Session=Depends(get_db)):
    ...
    user=identity_login(db,'google',profile['sub'],profile.get('email'),profile.get('name'),profile.get('picture'),parsed.get('uid'));return login_response(user.id,db)
...
async def github_callback(request:Request,code:str,state:str,db:Session=Depends(get_db)):
    ...
    user=identity_login(db,'github',profile['id'],email,profile.get('name') or profile.get('login'),profile.get('avatar_url'),parsed.get('uid'));return login_response(user.id,db)
...
def email_verify(request:Request,token:str,db:Session=Depends(get_db)):
    ...
    db.add(AuthTokenUse(token_hash=fingerprint));db.flush();email=parsed['email'];user=identity_login(db,'email',email,email,email.split('@')[0],None,parsed.get('uid'));return login_response(user.id,db)
...
def account_security(request:Request,db:Session=Depends(get_db)):
    ...
    return templates.TemplateResponse('security.html',{'request':request,'user':user,'csrf':session['csrf'],'bot_username':s.bot_username,'bot_runtime':getattr(request.app.state,'bot_runtime',{'online':False}),'identities':identities,'providers':providers(),'api_keys':keys,'two_factor':bool(security and security.enabled)})
...
def verify_two_factor(request:Request,code:str=Form(...),db:Session=Depends(get_db)):
    ...
    response=RedirectResponse('/dashboard',303);response.delete_cookie('blaze_preauth');response.set_cookie('blaze_session',sign_session(uid),httponly=True,secure=s.production,samesite='lax',max_age=s.session_ttl_seconds);return response
