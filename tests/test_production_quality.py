...
def test_migration_bootstrap_is_idempotent():
    ...
    assert first['revision']=='0003_upi_billing'
    assert 'alembic_version' in inspect(engine).get_table_names()
    with engine.connect() as connection:assert connection.execute(text('SELECT version_num FROM alembic_version')).scalar()=='0003_upi_billing'
...
def test_readiness_reports_migration_revision():
    with TestClient(app) as client:
        ...
        assert data['migration_state']=='ready' and data['database_revision']=='0003_upi_billing'
