...
@app.route('/health')
def health():
    return {"status": "ok", "running": len(bot_scripts), "users": len(active_users)}
...
def get_env(name, default=None, required=False, cast=str):
    ...
    # Empty string bhi treat karo as missing
    if value == "":
        value = None

    if required and value is None:
        raise ValueError(f"❌ ENV ERROR: {name} missing bhai 💀")
...
# Store pending ZIP files for approval - FIXED: Now disk persisted
pending_zip_files = {}  # {user_id: {file_name: file_path_on_disk}} - backward compat memory map
# Callback registry to fix 64-byte limit bug - maps short_id -> (owner_id, filename, original_type)
import hashlib
file_callback_registry = {}  # {short_id: {'owner': int, 'file': str, 'type': str, 'created': datetime}}
pending_approval_registry = {}  # {short_id: {'user_id': int, 'file_name': str, 'disk_path': str, 'kind': 'file|zip'}}

def generate_short_id(owner_id, file_name):
    """Generate short 8-char callback-safe ID and store mapping (fixes 64-byte limit)"""
    raw = f"{owner_id}_{file_name}_{time.time()}_{os.urandom(2).hex()}"
    short_id = hashlib.md5(raw.encode()).hexdigest()[:10]
    file_callback_registry[short_id] = {
        'owner': owner_id,
        'file': file_name,
    ...
    now = datetime.now()
    expired = [k for k, v in file_callback_registry.items() if (now - v['created']).total_seconds() > 3600]
    for k in expired:
        del file_callback_registry[k]
    return short_id

def resolve_short_id(short_id):
    """Resolve short_id to owner and filename"""
    data = file_callback_registry.get(short_id)
    if data:
        return data['owner'], data['file']
    return None, None

def generate_approval_id(user_id, file_name, disk_path, kind='file'):
    """Generate approval ID for admin buttons - disk persisted"""
    raw = f"approval_{user_id}_{file_name}_{time.time()}"
    short_id = hashlib.md5(raw.encode()).hexdigest()[:12]
    pending_approval_registry[short_id] = {
        'user_id': user_id,
        'file_name': file_name,
        'disk_path': disk_path,
        'kind': kind,
    ...
    meta_path = os.path.join(PENDING_DIR, f"{short_id}.json")
    try:
        ...
        logger.error(f"Failed to save approval meta {short_id}: {e}")
    return short_id

def load_pending_approvals():
    """Load pending approvals from disk on startup"""
    try:
        for fname in os.listdir(PENDING_DIR):
            if fname.endswith('.json'):
                ...
                try:
                    ...
                    data['created'] = datetime.fromisoformat(data.get('created', datetime.now().isoformat()))
                    pending_approval_registry[short_id] = data
                except Exception as e:
                    logger.error(f"Error loading pending {fname}: {e}")
        logger.info(f"Loaded {len(pending_approval_registry)} pending approvals from disk")
    except Exception as e:
        logger.error(f"Error loading pending approvals: {e}")

# Load on import after logger defined - will be called again after init_db
# (placeholder, actual call after logger setup)
...
init_db()
load_data()
# Load pending approvals from disk (FIXED persistence)
try:
...
def create_control_buttons(script_owner_id, file_name, is_running=True):
    """FIXED: Uses short callback IDs to avoid 64-byte limit"""
    short_id = generate_short_id(script_owner_id, file_name)
    # Store type info as well
    file_callback_registry[short_id]['type'] = 'control'
    markup = types.InlineKeyboardMarkup(row_width=2)
    if is_running:
        markup.row(
            types.InlineKeyboardButton("🔴 Stop", callback_data=f'stop_{short_id}'),
            types.InlineKeyboardButton("🔄 Restart", callback_data=f'restart_{short_id}')
        )
        markup.row(
            types.InlineKeyboardButton("🗑️ Delete", callback_data=f'delete_{short_id}'),
            types.InlineKeyboardButton("📜 Logs", callback_data=f'logs_{short_id}')
        )
    else:
        markup.row(
            types.InlineKeyboardButton("🟢 Start", callback_data=f'start_{short_id}'),
            types.InlineKeyboardButton("🗑️ Delete", callback_data=f'delete_{short_id}')
        )
        markup.row(
            types.InlineKeyboardButton("📜 View Logs", callback_data=f'logs_{short_id}')
        )
    markup.add(types.InlineKeyboardButton("🔙 Back to Files", callback_data='check_files'))
    # Also add legacy support button with full filename as fallback (for debugging)
    markup.add(types.InlineKeyboardButton(f"📄 {file_name[:20]}", callback_data='noop'))
    return markup

def create_file_button(script_owner_id, file_name, file_type, is_running):
    """Helper to create single file button with short ID (fixes 64-byte)"""
    short_id = generate_short_id(script_owner_id, file_name)
    file_callback_registry[short_id]['type'] = file_type
    status_icon = "🟢 Running" if is_running else "🔴 Stopped"
    btn_text = f"{file_name[:35]} ({file_type}) - {status_icon}"
    return types.InlineKeyboardButton(btn_text, callback_data=f'file_{short_id}')
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    """FIXED: Disk persistence for pending approvals + short callback IDs"""
    user_id = message.from_user.id
    user_folder = get_user_folder(user_id)
    temp_dir = None 
    try:
        ...
        # Security check for ZIP - NOTE: Filter logic kept as-is per instruction
        is_safe, security_msg = scan_zip_security(zip_path)
        if not is_safe:
            # FIXED: Save to disk instead of RAM so it survives restart
            pending_file_path = os.path.join(PENDING_DIR, f"zip_{user_id}_{int(time.time())}_{file_name_zip}")
            with open(pending_file_path, 'wb') as pf:
                pf.write(downloaded_file_content)
            # Generate short approval ID
            approval_id = generate_approval_id(user_id, file_name_zip, pending_file_path, kind='zip')
            
            security_warning_msg = f"🚨 File needs approval:\n👤 User: {user_id}\n📁 File: {file_name_zip}\n⚠️ Reason: {security_msg}\nID: `{approval_id}`"
            markup = types.InlineKeyboardMarkup()
            markup.row(
                types.InlineKeyboardButton("✅ Approve", callback_data=f"approve_zip_{approval_id}"),
                types.InlineKeyboardButton("❌ Reject", callback_data=f"reject_zip_{approval_id}")
            )
            for admin_id in admin_ids:
                try:
                    bot.send_message(admin_id, security_warning_msg, reply_markup=markup, parse_mode='Markdown')
                except Exception as e:
                    logger.error(f"Failed to send security warning to admin {admin_id}: {e}")
            
            # Keep backward compat mapping
            if user_id not in pending_zip_files:
                pending_zip_files[user_id] = {}
            pending_zip_files[user_id][file_name_zip] = pending_file_path
            
            bot.reply_to(message, f"⏳ File under security review. You will be notified upon approval.\nID: `{approval_id}`", parse_mode='Markdown')
            return
...
def process_zip_file(zip_path, user_id, user_folder, file_name_zip, message, temp_dir=None):
    """Process ZIP file extraction and setup - FIXED path traversal with commonpath"""
    cleanup_temp = False
    if temp_dir is None:
        temp_dir = tempfile.mkdtemp(prefix=f"user_{user_id}_zip_")
        cleanup_temp = True
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as zip_ref:
            # FIXED: Robust path traversal check using commonpath
            abs_temp = os.path.abspath(temp_dir)
            for member in zip_ref.infolist():
                member_path = os.path.abspath(os.path.join(temp_dir, member.filename))
                # Ensure member_path is within temp_dir using commonpath
                if os.path.commonpath([abs_temp, member_path]) != abs_temp:
                    raise zipfile.BadZipFile(f"Zip has unsafe path: {member.filename} (attempts directory traversal)")
                # Also block absolute paths and .. at start
                if member.filename.startswith('/') or member.filename.startswith('\\') or '..' in member.filename.split('/'):
                    # Still allow files in subfolders but not parent traversal
...
def _logic_check_files(message):
    ...
    for file_name, file_type in sorted(user_files_list):
        ...
        markup.add(create_file_button(user_id, file_name, file_type, is_running))
...
def handle_file_upload_doc(message):
    ...
    try:
        ...
        if file_ext == '.zip':
            ...
            # Security check for script files - NOTE: kept as-is per instruction
            is_safe, security_msg = check_code_security(file_path, file_ext[1:])
            if not is_safe:
                # FIXED: Use disk persistence + short ID to avoid 64-byte limit
                approval_id = generate_approval_id(user_id, file_name, file_path, kind='file')
                security_warning_msg = f"🚨 File needs approval:\n👤 User: {user_id}\n📁 File: {file_name}\n⚠️ Reason: {security_msg}\nID: `{approval_id}`"
                markup = types.InlineKeyboardMarkup()
                markup.row(
                    types.InlineKeyboardButton("✅ Approve", callback_data=f"approve_file_{approval_id}"),
                    types.InlineKeyboardButton("❌ Reject", callback_data=f"reject_file_{approval_id}")
                )
                for admin_id in admin_ids:
                    try:
                        bot.send_message(admin_id, security_warning_msg, reply_markup=markup, parse_mode='Markdown')
...
def file_control_callback(call):
    """FIXED: resolves short_id, supports legacy format, fixes permission"""
    try:
        data = call.data
        script_owner_id = None
        file_name = None
        
        # New format: file_<short_id>
        if data.startswith('file_'):
            payload = data[5:]
            # Try short_id resolution first
            owner, fname = resolve_short_id(payload)
            if owner is not None:
                script_owner_id = owner
                file_name = fname
            else:
                # Legacy fallback: file_<user_id>_<file_name>
                try:
                    _, sid, fname2 = data.split('_', 2)
                    script_owner_id = int(sid)
                    file_name = fname2
                except:
                    # Might be file_<short> where mapping expired -> try parse as legacy
                    pass
        
        if script_owner_id is None or file_name is None:
            logger.error(f"Could not resolve file callback {data}")
            bot.answer_callback_query(call.id, "⚠️ Session expired, reopen file list.", show_alert=True)
...
def start_bot_callback(call):
    """FIXED: short_id resolver + legacy fallback"""
    try:
        data = call.data
        short = data.replace('start_','',1)
        script_owner_id, file_name = resolve_short_id(short)
        if script_owner_id is None:
            # legacy: start_<owner>_<file>
            try:
                _, owner_str, fname = data.split('_',2)
                script_owner_id = int(owner_str)
                file_name = fname
            except Exception as e:
                bot.answer_callback_query(call.id, "⚠️ Session expired, reopen files.", show_alert=True)
                return
        requesting_user_id = call.from_user.id
...
def stop_bot_callback(call):
    """FIXED: short_id resolver"""
    try:
        data = call.data
        short = data.replace('stop_','',1)
        script_owner_id, file_name = resolve_short_id(short)
        if script_owner_id is None:
            try:
                _, owner_str, fname = data.split('_',2)
                ...
                bot.answer_callback_query(call.id, "⚠️ Session expired.", show_alert=True); return
        requesting_user_id = call.from_user.id
        chat_id_for_reply = call.message.chat.id
...
def logs_bot_callback(call):
    ...
    try:
        ...
        try:
            ...
            # FIXED: Escape markdown but keep readable
            if len(log_content) > 3500:
                # Send as document if too large
                tmp_log = os.path.join(user_folder, f"tmp_{file_name}.txt")
                with open(tmp_log, 'w', encoding='utf-8', errors='ignore') as tf:
                    tf.write(log_content)
                with open(tmp_log, 'rb') as tf:
                    bot.send_document(chat_id_for_reply, tf, caption=f"📜 Logs for `{file_name}` (User `{script_owner_id}`)")
                os.remove(tmp_log)
            else:
...
def process_broadcast_message(message):
    ...
    broadcast_content = message.text or message.caption or ""
...
def handle_confirm_broadcast(call):
    ...
    try:
        ...
        # Collect all possible content
        broadcast_data = {
            'text': original_message.text or original_message.caption,
            'photo': original_message.photo[-1].file_id if original_message.photo else None,
            'video': original_message.video.file_id if original_message.video else None,
            'document': original_message.document.file_id if original_message.document else None,
            'audio': original_message.audio.file_id if original_message.audio else None,
            'voice': original_message.voice.file_id if original_message.voice else None,
            'sticker': original_message.sticker.file_id if original_message.sticker else None,
            'caption': original_message.caption or None
        }
        # Check if has at least something
        if not any([broadcast_data['text'], broadcast_data['photo'], broadcast_data['video'], broadcast_data['document'], broadcast_data['audio'], broadcast_data['voice'], broadcast_data['sticker']]):
            raise ValueError("Message has no supported content for broadcast.")

        bot.answer_callback_query(call.id, "🚀 Starting broadcast...")
        bot.edit_message_text(f"📢 Broadcasting to {len(active_users)} users... This may take a while.",
                              chat_id, call.message.message_id, reply_markup=None)
        thread = threading.Thread(target=execute_broadcast, args=(broadcast_data, chat_id))
        thread.start()
...
def execute_broadcast(broadcast_data, admin_chat_id):
    """FIXED: supports all media types, handles flood control, markdown fallback"""
    ...
    for i, user_id_bc in enumerate(users_to_broadcast):
        try:
            ...
            # Try send in priority order - one media per broadcast (original logic)
            if broadcast_data.get('document'):
                bot.send_document(user_id_bc, broadcast_data['document'], caption=caption, parse_mode='Markdown' if caption else None)
            elif broadcast_data.get('photo'):
                bot.send_photo(user_id_bc, broadcast_data['photo'], caption=caption, parse_mode='Markdown' if caption else None)
            elif broadcast_data.get('video'):
                bot.send_video(user_id_bc, broadcast_data['video'], caption=caption, parse_mode='Markdown' if caption else None)
            elif broadcast_data.get('audio'):
                bot.send_audio(user_id_bc, broadcast_data['audio'], caption=caption, parse_mode='Markdown' if caption else None)
            elif broadcast_data.get('voice'):
                bot.send_voice(user_id_bc, broadcast_data['voice'], caption=caption, parse_mode='Markdown' if caption else None)
            elif broadcast_data.get('sticker'):
                bot.send_sticker(user_id_bc, broadcast_data['sticker'])
            elif broadcast_data.get('text'):
                # Try markdown, fallback to plain if fails
                try:
                    bot.send_message(user_id_bc, broadcast_data['text'], parse_mode='Markdown')
                except:
                    bot.send_message(user_id_bc, broadcast_data['text'])
            sent_count += 1
        except telebot.apihelper.ApiTelegramException as e:
            ...
            if any(s in err_desc for s in ["bot was blocked", "user is deactivated", "chat not found", "kicked from", "restricted"]): 
                ...
                try:
                    if broadcast_data.get('text'):
                        ...
                        # Retry with first available media without markdown
                        if broadcast_data.get('photo'):
                            bot.send_photo(user_id_bc, broadcast_data['photo'], caption=broadcast_data.get('caption'))
                        elif broadcast_data.get('document'):
                            bot.send_document(user_id_bc, broadcast_data['document'], caption=broadcast_data.get('caption'))
                    sent_count += 1
                except Exception as e_retry: 
                    logger.error(f"Broadcast retry failed to {user_id_bc}: {e_retry}"); failed_count +=1
            else: 
...
def bot_performance_callback(call):
    ...
    try:
        ...
        ratio = (running_scripts/total_files*100) if total_files>0 else 0
        performance_parts.append(f"• Uptime Ratio: {running_scripts}/{total_files} ({ratio:.1f}% if total > 0)")
...
def cleanup_files_callback(call):
    """FIXED: Also cleans pending approvals + temp files"""
    ...
    try:
                ...
                if not os.listdir(user_path):
                    ...
                    for file_name in os.listdir(user_path):
                        if file_name.endswith('.log'):
                            ...
                            try:
                                os.remove(os.path.join(user_path, file_name))
                                cleaned_files += 1
                            except: pass
        
        # Clean old pending approvals (>1 day)
        try:
            now = time.time()
            for fname in os.listdir(PENDING_DIR):
                fpath = os.path.join(PENDING_DIR, fname)
                try:
                    ...
                    if age > 24*3600:
                        ...
                        sid = fname.replace('.json','').replace('.bin','')
                        if sid in pending_approval_registry:
                            del pending_approval_registry[sid]
                except Exception as e:
                    logger.error(f"Error cleaning pending {fpath}: {e}")
        except Exception as e:
            logger.error(f"Pending cleanup error: {e}")

        # Clean file_callback_registry old entries
        try:
            now = datetime.now()
            expired = [k for k, v in file_callback_registry.items() if (now - v['created']).total_seconds() > 3600]
            for k in expired:
        ...
        result_msg = f"🧹 **Cleanup Complete:**\n• Empty dirs: {cleaned_dirs}\n• Old logs/tmp: {cleaned_files}\n• Old pending: {cleaned_pending}\n• Callback cache cleaned"
...
def process_approve_file(call):
    """FIXED: Supports both new approval_id and legacy user_id_file_name"""
    try:
        payload = call.data.replace('approve_file_','',1)
        # Try new system first
        approval_data = pending_approval_registry.get(payload)
        if approval_data:
            user_id = approval_data['user_id']
            file_name = approval_data['file_name']
            file_path = approval_data['disk_path']
        else:
            # Legacy fallback: approve_file_<user_id>_<file_name>
            # Try load from disk meta
            meta_path = os.path.join(PENDING_DIR, f"{payload}.json")
            if os.path.exists(meta_path):
                ...
                # Old style: approve_file_<user_id>_<file>
        ...
        user_folder = get_user_folder(user_id)
        file_path_actual = file_path if os.path.exists(file_path) else os.path.join(user_folder, file_name)
        
        if not os.path.exists(file_path_actual):
        ...
        try:
            if file_ext == '.js':
                handle_js_file(file_path_actual, user_id, user_folder, file_name, call.message)
            elif file_ext == '.py':
                handle_py_file(file_path_actual, user_id, user_folder, file_name, call.message)
            ...
            # Cleanup approval registry and meta
            if payload in pending_approval_registry:
                del pending_approval_registry[payload]
            meta_path = os.path.join(PENDING_DIR, f"{payload}.json")
            if os.path.exists(meta_path):
        ...
        logger.error(f"Error in process_approve_file: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "❌ Error.", show_alert=True)

def process_reject_file(call):
    """FIXED: supports new ID"""
    try:
        payload = call.data.replace('reject_file_','',1)
        approval_data = pending_approval_registry.get(payload)
        if approval_data:
            user_id = approval_data['user_id']
            file_name = approval_data['file_name']
            disk_path = approval_data['disk_path']
            # Delete file
            ...
            if os.path.exists(meta_path):
                ...
                file_name = '_'.join(parts[1:]) if len(parts)>1 else payload
                user_folder = get_user_folder(user_id) if user_id else None
                if user_folder:
                    file_path = os.path.join(user_folder, file_name)
                    if os.path.exists(file_path):
        ...
        logger.error(f"Error in process_reject_file: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "❌ Error.", show_alert=True)

def process_approve_zip(call):
    """FIXED: supports approval_id + disk persistence"""
    try:
        payload = call.data.replace('approve_zip_','',1)
        approval_data = pending_approval_registry.get(payload)
        if not approval_data:
            ...
            if os.path.exists(meta_path):
                ...
                # Legacy: approve_zip_<user_id>_<file>
                parts = payload.split('_')
                if len(parts)>=2 and parts[0].isdigit():
                    ...
                    if user_id in pending_zip_files and file_name in pending_zip_files[user_id]:
                        content_path = pending_zip_files[user_id][file_name]
                        if isinstance(content_path, str) and os.path.exists(content_path):
                            # it's disk path now
                            approval_data = {'user_id': user_id, 'file_name': file_name, 'disk_path': content_path}
                        else:
                            # old bytes content
                            temp_p = os.path.join(PENDING_DIR, f"legacy_{user_id}_{file_name}")
                            if isinstance(pending_zip_files[user_id][file_name], bytes):
                                with open(temp_p,'wb') as f:
                                    f.write(pending_zip_files[user_id][file_name])
                                approval_data = {'user_id': user_id, 'file_name': file_name, 'disk_path': temp_p}
                    else:
                        bot.answer_callback_query(call.id, "❌ File not found, ask re-upload.", show_alert=True)
                        return
                else:
                    bot.answer_callback_query(call.id, "❌ Invalid data.", show_alert=True)
                    return
        ...
        if not os.path.exists(disk_path):
            bot.answer_callback_query(call.id, "❌ ZIP file not found on disk.", show_alert=True)
            return
        
        user_folder = get_user_folder(user_id)
        temp_dir = None
        try:
            # Read content for process_zip_file flow
        ...
        logger.error(f"Error in process_approve_zip: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "❌ Error.", show_alert=True)

def process_reject_zip(call):
    """FIXED: supports new ID"""
    try:
        payload = call.data.replace('reject_zip_','',1)
        approval_data = pending_approval_registry.get(payload)
        file_name = "archive"
        user_id = None
        if approval_data:
            ...
            if os.path.exists(meta_path):
                ...
                if parts[0].isdigit():
                    user_id = int(parts[0])
                    file_name = '_'.join(parts[1:])
                if user_id and user_id in pending_zip_files and file_name in pending_zip_files[user_id]:
                    dp = pending_zip_files[user_id][file_name]
                    if isinstance(dp, str) and os.path.exists(dp):
                        try: os.remove(dp)
                        except: pass
                    del pending_zip_files[user_id][file_name]
        ...
        logger.error(f"Error in process_reject_zip: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "❌ Error.", show_alert=True)
...
if __name__ == '__main__':
    logger.info("="*50 + "\n🤖 BLAZE NXT Hosting Bot Starting Up (FIXED VERSION)...\n" + f"🐍 Python: {sys.version.split()[0]}\n" +
                f"🔧 Base Dir: {BASE_DIR}\n📁 Upload Dir: {UPLOAD_BOTS_DIR}\n" +
                f"📊 Data Dir: {IROTECH_DIR}\n🔑 Owner ID: {OWNER_ID}\n🛡️ Admins: {len(admin_ids)}\n" +
                f"🚫 Banned Users: {len(banned_users)}\n📢 Mandatory Channels: {len(mandatory_channels)}\n" + "="*50)
    keep_alive()
    logger.info("🚀 Starting polling (fixed version)...")
    while True:
        try:
