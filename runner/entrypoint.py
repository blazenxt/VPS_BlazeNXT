...
base=os.environ.pop('CONTROL_PLANE_URL').rstrip('/');wid=os.environ['WORKLOAD_ID'];token=os.environ.pop('RUNNER_TOKEN')
...
if runtime=='python':
    venv=pathlib.Path('/workspace/venv');subprocess.run([sys.executable,'-m','venv',str(venv)],check=True)
    python=str(venv/'bin/python')
    if pathlib.Path('requirements.txt').is_file():subprocess.run([python,'-m','pip','install','--no-cache-dir','-r','requirements.txt'],check=True,timeout=300)
    cmd=[python,'-I',entry]
else:
    if pathlib.Path('package.json').is_file():subprocess.run(['npm','install','--omit=dev','--ignore-scripts'],check=True,timeout=300)
    cmd=['node','--disable-proto=delete',entry]
os.execvp(cmd[0],cmd)
